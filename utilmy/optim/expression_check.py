...
Issues:
1)
The test6 function is highly hard coded and will requires substantial changes so maybe 
...
And the test7 can be made with some modifications in search_formulae_dcgpy_Xy_regression_v1, 
but results are not consistent in this so maybe we need to tweak parameters such as number
of evolution to get.
Reference: http://darioizzo.github.io/dcgp/notebooks/symbolic_regression_2.html

2)With increase in number of constants, the final expression changes,
so needed to increase the number of evaluations to get same results.

3) test1 has some error dealing with constants so will be improved
...
def test7():
    ...
    # Input your ytrue expressions here
    ...
    def search():
        ...
        for i in range(n_exp):
            dCGP = expression(inputs=nvars_in, outputs=nvars_out, rows=1, cols=15, levels_back=16, arity=2,
                              kernels=kernels_new,n_eph = n_eph,
