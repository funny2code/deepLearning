...
ls -l /sys/fs/cgroup/cpa/db/uvector.cache

rm -rf   /sys/fs/cgroup/cpa/db/uvector.cache

cp -R  /a/gfs101/ipsvols07/ndata/cpa/db/db20220211_keep/uvector.cache   /sys/fs/cgroup/cpa/db/
...
if 'import':
    ...
    with warnings.catch_warnings():
        ...
        import os, glob, sys, math, string, time, json, logging, functools, random, numpy as np, pandas as pd,  h5py, yaml, operator, gc
        # import scipy
        import  matplotlib
        import copy, mmh3
...
if 'db diskcache':
    ...
    db = Box({
        ...
        'db_easyid_topgenre_repur' :  f"{db_dir}/db_easyid_topg_freqpur.cache",    ### Intra updates
...
if 'utils' :
    ...
    def rename():   ## python prepro.py rename
         ...
         for fi in flist :
            ...
            if  '.parquet' in fi.split("/")[-1]   : continue
...
if 'backup' :
    ...
    def backup_code_daily():   # py backup_code_daily &
        t0      = str(date_now_jp() )
        tk = get_timekey()
        dirin   =  "/home/noelkevin01/test_code/recsys"
        dirout1 = f"/a/gfs101/ipsvols07/ndata/cpa/hdfs/daily_usertopk/m001/{tk}/code"
        log(t0, dirin, dirout1)
        try :
          os.rename( dirout1, dirout1 + "_" + str( int(time.time()) ) )
        except: pass        
        os_copy_safe(dirin, dirout1, nlevel=1, nfile=5000, logdir="./", cmd_fallback= cmd_mountgs,  pattern="*.py" )
...
if 'daily_check':
    def daily_check_batches(add_days=0):   ###   py2 daily_check_batches > "${logfile}_00_daily_batch_check.py"  2>&1  &
      ...
      tk2,tk1,tk = get_timekey()-2 + add_days , get_timekey()-1 + add_days ,  get_timekey() + add_days
      ...
      log("\n\n\n")
      flist = glob_glob( dir_cpa3 + f"/hdfs/daily_user_eod/{tk-1}/sc_stream_intra/*.parquet", 1  )
      df = pd_read_file( flist[0] )
      df = pd_add_easyid_bk( df )    
      log( 'daily_user_eod, bk unique', df.bk.unique()  )
        
        
      log("\n\n")  
      flist = glob_glob( dir_cpa3 + f"/hdfs/daily_user_hist/{tk-1}/brw_ran/*.parquet", 1  )
      df = pd_read_file( flist[0] )
      df = pd_add_easyid_bk( df )    
      log( 'daily_user_hist , bk unique', df.bk.unique()  )
...
if 'embed':
    ...
    def item_add_text_vector(modelin=None, dirin=None, dirout=None, nrows=500500500, nfile=1000, today=None,
        ...
        flist = [ fi  for fi in flist if not os.path.isfile(dirout + fi.split("/")[-1].replace(".csv", ".parquet") ) ]
        if len(flist) < 1: 
            log('Empty') ; return len(flist)
        ...
        for ii,fi in enumerate(flist) :
            ...
            log(  df[[ 'item_id', 'item_emb'  ]].head(3) )
...
if 'cassandra':
    ...
    def pd_cass_get_vect2(df, prefix="m001", tablename="ndata.item_model", ivect=None, update_cache=False )  :
        ...
        df = pd_add_siid(df,  delete=False)    
        # if 'siid' not in df : df['siid'] = df.apply( lambda x : siid(x), axis=1)
        siids  = set( df['siid'].values[:] )
        ...
        siids['item_emb'] = siids['item_emb'].astype('str')
        log( 'Query Res:', len(siids), str(siids)[:45] )

        
        #### Bad or missing siids
        tk  = get_timekey()
        df1 = siids[siids.item_emb.str.len()<200][[ 'siid', 'item_emb'  ]]
        if len(df1) > 0:
           pd_to_file(df1, (dir_cpa3 + f"/hdfs/intraday/item_bademb/{tk}/siid_{int(time.time())}_{len(df1)}.parquet") , show=0)        
           siids = siids[siids.item_emb.str.len()>200]        
        return siids
...
if 'user_hist':
    def pd_easyid_gettoday(add_days=0, n_hours=0):
       #### Filter by today
       today1 = date_now_jp(add_days= 0 + add_days)
       flist =  glob_glob(dir_cpa3 + f"/hdfs//intraday/sc_stream/{today1}/*brow*0.parquet", 100)
       flist = sorted(flist)[-15:]
       dfeasy = pd_read_file( flist, cols=['easy_id'] , drop_duplicates=['easy_id'], npool=20)
       dfeasy = set( dfeasy['easy_id'].unique())
       log('Neasy to update', len(dfeasy) )
       return dfeasy  
...
if 'user_emb':
    ...
    def daily_useremb_update_cass(bmin=0, bmax=500, split=0, add_days=0, full=0):  
        ###  py daily_useremb_update_cass --split 10  --add_days 1  --full 1 >> "${logfile}_useremb_cass7.py"  2>&1  & 
        ### 3 hours to update
        tk     = get_timekey() -1 + add_days
        today1 = date_now_jp(add_days= -1 + add_days)
        
        if split > 0 :
            cmd = f"{PYFILE} daily_useremb_update_cass --split 0  --add_days {add_days} --full {full}    "
            batch_split_run(cmd= cmd, split=split, sleep= (15, 30) ) ; return 'done'
        
        if full < 1 :
           dfeasy = pd_read_file2( dir_cpa3 + f"/hdfs//intraday/sc_stream/{today1}/*brow*0.parquet", cols=['easy_id'] , drop_duplicates=['easy_id'], npool=10)
           dfeasy = set( dfeasy['easy_id'].unique())
           log('Neasy to update', len(dfeasy) )
        ...
        for bk in range(bmin, bmax):
           ...
           df = pd_read_file(fi)         
           if full < 1 : df = df[df.easy_id.isin(dfeasy)]
           log(df.shape)
           cass_update2(df[[ 'easy_id', 'user_emb'  ]], table='ndata.user_model', prefix=model_pref, colkey="easy_id", colval="user_emb",
                        cass_query=cass_query, ttl=86400*100, consistency='quorum')  ## 240days
                        

    def cass_update2(df, table, prefix="m001", colkey="siid", colval="siid_emb", cass_query=None, ttl=21086400, kbatch=10, consistency = 'one'):  ## 240days
       ### Very Slow to insert (!)
       log("Start cass insert", table, colkey, colval)
       df = df[[colkey, colval]].drop_duplicates(colkey)
       df = df.values
       ...
       for i in range(len(df)):
            ...
            isok = cass_fun_upsert( cdata, ttl_sec= ttl, consistency = consistency, istest=False)
            cdata = {}
...
if 'rec_genre':
    ...
    def create_rec_topgenre(df1, ngenre=5, topk=6):
        ...
        #### Later storage   
        # df1['top_genre_intra'] = df1['easy_id'].apply(lambda x: db_easyid_topgenre_intra.get(x, "") )        

        
        ff = { i : np.exp(-0.005*i) for i in range(0,20) }  ### Adjustement factor for top-genre weighting
    ...
    def easyid_create_daily_genre(x, ca_genreid=None, nmax=40, ff=None, **kw):
        ...
        for i, gi in enumerate( db_easyid_topgenre_pur.get(easyid, "").split(";")[0].split(",") ) :
          if gi in  ca_genreid and i < 20:
             gall[gi] = 1.75*ff[i]   if gi not in gall else gall[gi] + 1.75*ff[i]
        ...
        for i,gi in enumerate(db_easyid_topgenre_brw.get(easyid, "").split(";")[0].split(",") ):
          if gi in  ca_genreid and i < 20:
             ...
             gall[gi] = 2.0*ff[i]  if gi not in gall else gall[gi] + 2.0*ff[i]

                
        for i,gi in enumerate(db_easyid_topgenre_intra.get(easyid, "").split(";")[0].split(",")) :
          if gi in  ca_genreid and i < 20:
             gall[gi] = 0.75*ff[i] if gi not in gall else gall[gi]  + 0.75*ff[i]

        for i,gi in enumerate(db_easyid_topgenre_repur.get(easyid, "").split(";")[0].split(",")) :
          if gi in  ca_genreid and i < 20:
             gall[gi] = 1.0*ff[i] if gi not in gall else gall[gi]  + 1.0*ff[i]
...
if 'repurchase':
    def daily_db_easyid_repur( add_days=0 , bmin=0, bmax=500, split=0):   ### py daily_db_easyid_repur --add_days -1    >> "${logfile}_db_repur_update.py"  2>&1
        """ /a/gfs101/ipsvols07/ndata/cpa/hdfs/repur/
                gms  n_pur         price             siid  units

           repur/genreid : easy_id  genre_id  repur_rate  units

        """
        tk = get_timekey() -2 + add_days

        dirin  = dir_cpa3 +"/hdfs/repur/genreid/{tk}/*_{bk}_*.parquet"
        flist  = glob_glob(dirin.format(tk=tk, bk="*") )

        flist2 = []
        for ii,fi in enumerate(flist) :
            # if ii > 11: break
            flist2.append(fi)
            if len(flist2) < 10  and  ii < len(flist) - 1: continue
            df = pd_read_file(flist2, npool=10) ;
            log(df.shape) ; flist2 = []
            df = df.query('repur_rate >  0.05')   ###5k  10%

            df['genre_id'] = df['genre_id'].astype('int32')
            df['easy_id']  = df['easy_id'].astype('int64')

            df = df.groupby('easy_id').apply(lambda dfi :  ",".join( [  str(t) for t in   dfi.genre_id.values ]   )   ).reset_index()
            df.columns = ['easy_id', 'genre_list']
            npool = 1
            # df = df.iloc[:100, :]
            diskcache_save2(df, 'easy_id', colvalue='genre_list', db_path= db.db_easyid_topgenre_repur, npool= npool, ttl= None, verbose=False )
...
if 'rec_item_emb':
    #############################################################################################################
    def  daily_eod_user_hist_item(add_days=-1, tag=""):  ###  py  daily_eod_user_hist_item    >> "${logfile}_histitem.py"  2>&1   &
        tk     = get_timekey()  + add_days
        ...
        for bi in df['bk'].unique():
            dfi = df[df.bk == bi ]
            pd_to_file(dfi, dirout + f"/siid_list_{bi}_{len(dfi)}.parquet", show=0)
    ...
    def pd_item_topk_update(df, faiss_index=None, faiss_map_idx=None, ivect=None, dgenre=None, overwrite=True, npool=1, append=False ):
        ...
        df['topk'] = df.apply(lambda x : pd_siid_cleangenre3(x, topk=40, dgenre=dgenre) , axis=1)   #### need db_ca_siid_genre
        log(df[[ 'siid', 'topk' ]])
    ...
    def pd_add_itememb(df, delete=True, show=0):     
        #### Add item emb from siid
        df = pd_add_itemaster(df)    ### Too much ressources on Cass        
        df['item_text'] = df.apply(lambda x : item_merge_field(x), axis=1)
        df = df[df.item_text.str.len() > 100 ]        
        if len(df) < 1: return df

        model  = text_model_load(model0['dir'] )        
        df['item_emb']  = text_tovec_batch(model, df['item_text'].values )
        if delete : df = df[[ 'siid', 'item_emb' ]]
        if show>0: log(df)
        return df
    ...
    def create_rec_topemb3(df, faiss_index=None, faiss_map_idx=None, ivect=None, maxrec=30, dfe=None):
        ...
        """
        def get_topk(siids, easyid=None):
            topks = [] ; jj = -1
            
            #### Re-organize if  genreid in Repur-table 
            glist0 =  db_easyid_topgenre_repur.get(easyid, '')
            # glist0 =  db_easyid_topgenre_brw.get(easyid, '')
            
            if len(glist0) > 1:
                glist0 = glist0.split(",")
                for sid in siids :
                    slist = db_item_toprank.get(sid, "").split(",")
                    gid   = db_ca_siid_genre.get(sid, '-')                    
                    if gid in glist0 :
                        jj = jj + 1
                        topks.insert(jj, slist)
                    else :    
                        topks.append( slist )
            else :            
                for sid in siids :
        ...
        if dfe is not None:  #### Add User Embedding  user_emb
           df             = df.merge(dfe, on='easy_id', how='left')
           log(df, 'no-NA user emb: ', len(df[-df.user_emb.isna() ]) )
           df['user_emb'] = df['user_emb'].fillna('0')   ### user with NO embeddings
           df['user_emb'] = df['user_emb'].apply(lambda vi:  np.array([ float(x) for x in vi.split(",")] ,  dtype='float32') )
        ...
        df['topk'] = df.apply(lambda x: get_topk(x['siids'], x['easy_id'] ) , axis=1)        
        log(df.head(5))
        return df
...
if 'genre_predict':    
    def daily_eod_user_topg_dump(bmin=0, bmax=500, add_days=0, split=0, full=1):  ### py2 daily_eod_user_topg_dump   >> "${logfile}_eod_top_dump9.py"  2>&1  &
        ### Dump on disk top genre
        ...
        dirout  = dir_cpa3 + f"/hdfs/daily_usertopk/m001/{tk}/top_genre/"
        log(dirin,  dirout)
        
        if split > 0 :
            os_rename(dirout,)  ### Need to rename existing one 
            cmd = f"{PYFILE} daily_eod_user_topg_dump --split 0  --add_days {add_days}  --full {full}   "
            batch_split_run(cmd= cmd, split=split, sleep= (2, 5) ) ; return 'done'
                      
        ii = 0    
        for bk in range(bmin, bmax):
           ii = ii + 1 
           df = pd_read_file(dirin + f"*_{bk}_*.parquet", cols=['easy_id', 'top_genre', 'topk'], n_pool=1)
           df = df.drop_duplicates('easy_id')        
        
           df['genre_intra'] = df['easy_id'].apply(lambda x : db_easyid_topgenre_intra.get(x, '').replace(";", "")  )
           df['genre_brw']   = df['easy_id'].apply(lambda x : db_easyid_topgenre_brw.get(x,   '').replace(";", "")  )
           df['genre_pur']   = df['easy_id'].apply(lambda x : db_easyid_topgenre_pur.get(x,   '').replace(";", "")  )
           df['genre_repur'] = df['easy_id'].apply(lambda x : db_easyid_topgenre_repur.get(x, '').replace(";", "")  )
        
           if ii < 5: 
             log(df, df.columns,)
             log( 'brw', df.genre_brw.str.len().mean() , 'intra',  df.genre_intra.str.len().mean() , 'pur',  df.genre_pur.str.len().mean() ,   )
            
           dirouti =  dirout + f"/user_eod_topgenre_{bk}_{tk}_{today}_{len(df)}.parquet"
           pd_to_file(df,  dirouti, show=0)
        log(df.columns)         
    ...
    def date_unix(add_days=0, hours=7, mins=17 ):
        ...
        now = now + datetime.timedelta(days=add_days)
        log(now)
        return time.mktime(now.utctimetuple())


    def metric_genre(df):    
        log("##### Hit rate per genre")
        df['y_genre_pur2'] = df.apply(lambda x :  x['y_genre_pur'].split(","), axis=1)

        def hit_calc(ss1, ss2):
            if len(ss1)< 1:
                return np.nan
            ss1 = ss1.split(",")
            # ss2 = split(",")
            if len(ss1) > 0 and len(ss2) >0 :
                return len( set(ss1).intersection( set(ss2) )) / len(ss2)
            return -1 

        ll = [  'genre_intra', 'genre_brw', 'genre_pur', 'genre_repur' , 'top_genre' ]
        for ci in ll :
            ci2 = 'hit_' + ci             
            if ci in df.columns :
               df[ci2] = df.apply(lambda x : hit_calc(x[ci], x['y_genre_pur2']), axis=1)
            else :
               df[ci2] = 0.0 
            
        df1 = df[ (df.hit_genre_brw >0.0 ) & (df.hit_genre_pur >0.0 )   ]             
        res = [[ 'size' , len(df1), len(df) ]] ; nn = len(df)
        for ci in ll :
            res.append([ ci, df1[ 'hit_' + ci].mean(), df[df['hit_' + ci] > -1.0 ].shape[0] / nn ] )

        res = pd.DataFrame(res, columns= ['name', 'hitrate', 'coverage'])
        log( res )
        del df['y_genre_pur2']    
        return df    

            
    def pd_read_filter(dirin, filter_col:str, filter_vals:set, nfile=5000, kbatch=50, show=1) :
        """  Read files  and filter it.
        """
        flist  = glob_glob( dirin , nfile=nfile)
        fj = []; df2 = pd.DataFrame()
        for ii, fi in enumerate(flist):
            fj.append(fi)
            if len(fj) < kbatch  and ii < len(flist)-1: continue
            dfk = pd_read_file(fj, npool=20)
            dfk = dfk[dfk[filter_col].isin(filter_vals) ]
            log(ii, dfk.shape)
            df2 = pd.concat((df2, dfk ))
            fj = []

        if show>0: log(df2.shape)
        return df2


    def daily_check_genrepredict(add_days=0, bmin=0, bmax=500, split=0, tag="scpur", nfile=20,  mode='eod', iscouch=True):   ####
        """   py  daily_check_genrepredict   --add_days -1   >> "${logfile}_genrepredict.py"  2>&1   &
        ...
        dirout = dir_cpa3 + f"/hdfs/daily_usertopk/m001/{tk}/top_genre_pred/"

        log("#### Purchase easyid")
        df   = pd_read_file(dir_cpa3 + f"/hdfs/intraday/sc_stream/{today}/*purchase*0.parquet", npool=20,  nfile=100 )
        tmax = date_unix(add_days=add_days, hours=14, mins=0 )
        df = df[df.ts > tmax]
        # df = df.iloc[:20000, :]

        df = pd_add_siid(df, delete=True)
        df = df.groupby('easy_id').apply(lambda dfi : ",".join([ str(t) for t in dfi.genre_id])  ).reset_index()
        df.columns =['easy_id', 'y_genre_pur']
        df = pd_add_easyid_bk(df)
        df = df.sort_values('bk')
        leasyid = set( df.easy_id.unique() )
        log(df)

        
        log("#### Load top_genre, genre_brw,... ")
        dirin1 = dir_cpa3 + f"/hdfs/daily_usertopk/m001/{tk}/top_genre/*.parquet"
        df2 = pd_read_filter(dirin1, filter_col='easy_id', filter_vals=leasyid, nfile=5000, kbatch=50, show=1)

        df = df.merge(df2, on='easy_id', how='left')   ; del df2
        df = df.fillna('')
        df = metric_genre(df)

        log( 'ngenre', len( df[ df.genre_brw.str.len() > 5] ) )
        log(df.columns)
        pd_to_file(df, dirout +f"/user_pur_topgenre_{len(df)}.parquet", show=1)
...
if 'daily_afternoon':
    ###### Afternoon Batch  #####################################################
    def daily_eod_user_hist(ii=0, ii2=500, tag='all', add_days=0):   ### py  daily_eod_user_hist --add_days 0  --ii2 3   >> "${logfile}_eod_user.py"  2>&1
        """    python prepro_prod.py  daily_eod_user_hist    --ii 0    &
               ### Other 15 mio users,  20min to run
        """
        ...
        for bk in range(bkmin, bkmax):
            ...
            while len(df1) < 1000:
                ...
                dirin2 = dir_cpa3 + f"/hdfs/daily_user_hist/{tj}/brw_ran//*_{bk}_*.parquet"                
                df1    = pd_read_file( dirin2 , nfile=1)

                dirin2 = dir_cpa3 + f"/hdfs/daily_user_hist/{tj}/pur_ran//*_{bk}_*.parquet"                
                df1i   = pd_read_file( dirin2 , nfile=1)
                df1    = pd.concat((df1i, df1))
            ...
            df2 = pd.DataFrame()
            for fi in flist:
               dfi = pd_read_file(fi, cols=cols, n_pool=1,  )      ### Need to preserve order
               df2 = pd.concat(( df2, dfi ))
            log(df2.shape)

            #df2  = df2.drop_duplicates([ 'easy_id', 'shop_id', 'item_id' ])

            log('Nincorrect', len(dfr))
            if 'genre_id' not in df1.columns :  df1['genre_id'] = 0
            if 'bk' in dfr.columns: df2 = pd.concat((df1[cols], dfr[dfr.bk == bk ][cols], df2[cols],  ))
            else :                  df2 = pd.concat((df1[cols], df2[cols],  ))

            df2 = pd_add_easyid_bk(df2)            
            dfr = pd.concat(( dfr,  df2[df2.bk != bk ] ))  ### Residual incorrect
            
            df2 = df2[df2.bk == bk ]  ### Correct
            df2 = df2.drop_duplicates( ['easy_id', 'genre_id'] , keep='last')
            df2 = df2.groupby('easy_id' ).tail(5)  #### Last 5 events

            neasy = df2.easy_id.nunique()
            pd_to_file(df2, dirouti.replace(".parquet", f"_{len(df2)}_{neasy}.parquet"), show=1 )



    def daily_eod_cass_item(mode='pur', tmin=-10, tmax=-1):   ### py  daily_eod_item  >> zlog_daily5b.py
        """   sleep 600 &&  python prepro_prod.py daily_eod_item --mode pur > zlog_daily6a.py  2>&1   &
                    py  daily_eod_cass_item --mode brw >  "${logfile}_eod_item_brw3.py"  2>&1  &

                    py  daily_eod_item --mode "pur,custom"  --tmin -175  --tmax -30   >  "${logfile}_eod_item_pur_custom.py"  2>&1  &
        """
        # tk      = get_timekey() - 7
        if 'pur' in mode :    tk_list = [ t for t in range(get_timekey()-1, get_timekey() - 10 , -1)  ]
        if 'brw' in mode :    tk_list = [ t for t in range(get_timekey()-1, get_timekey() - 3  , -1)  ]
        ...
        model  = text_model_load(model0['dir'] )
        dirout = dir_cpa3 + f"/hdfs/zlog/"    ## Related to model
        index  = Index1( dirout + "/daily_item_cassupdate_done.txt"  )
        ...
        for tk in tk_list :
            ...
            for dirin in dirin_list :
                ...
                for kk,fi in enumerate(flist) :
                    ...
                    # df2 = df2.iloc[:100, :]
                    # if len(df2) < 11000000 and kk < len(flist)-1 : continue
                    log('N siids',  df2.shape)
    ...
    def daily_create_easyid_topg2(ii=0, ii2=500, tag='all'):     ## py  daily_create_easyid_topg2  --tag brw   >> zlog_genre.py
        """
        """
        t0   = get_timekey() - 1
        tmax = get_timekey()
    ...
    def intra_user_hist():  ### py  intra_user_hist
    ...
    def daily_item_info(mode='create'):  ####   py  daily_item_info --mode brw  &
    ...
    def daily_eod_user_histfull(bmin=0, bmax=500, tag='brw', add_days=0, past= -1, mode='', split=0):   ###
        """ Past cathcup, 2hours to finish
           py  daily_eod_user_histfull   --split 0 --past -300  --tag pur  --add_days 0    --mode full    >> "${logfile}_user_histofull_pur2.py"  2>&1   &
...
if 'daily_rec_topk':
    ...
    def daily_eod_user_hist_intra(add_days=-1):  ### python prepro_prod.py  daily_eod_user_hist_intra
        ...
        while len(df2) < 1 :
           ...
           df2    = pd_read_file2(dirin2, n_pool=1, nfile=1)  ; log(df2)
        ...
        log(dirin)
        cols   = ['easy_id',  'item_id',  'shop_id', 'genre_id', 'ts', ]
        df     = pd_read_file2( dirin , cols=cols, nfile=1000, n_pool=25)
        df = df.sort_values(['easy_id', 'ts' ])
        df = df.drop_duplicates( ['easy_id', 'genre_id'] , keep='last')
        ...
        log('\n ####### Load Intraday T-2')
        dirin  = dir_cpa3 + f"/hdfs/intraday/sc_stream/{today2}/*.parquet"        
        dfb    = pd_read_file2( dirin , cols=cols, nfile=1000, n_pool=25, )
        log(dirin, dfb.shape)
        dfb = dfb.sort_values(['easy_id', 'ts' ])
        dfb = dfb.drop_duplicates( ['easy_id', 'genre_id'] , keep='last')

        df = pd.concat((dfb, df)) ; del dfb        
        df = df.drop_duplicates( ['easy_id', 'genre_id'] , keep='last')        
        ...
        log('######## Save on disk')
        for bk in range(0, 500):            
           dirin2 = dir_cpa3 + f"/hdfs/daily_user_eod/{ti}/brw_ran_v15/*_{bk}_*.parquet" 
           df2    = pd_read_file2(dirin2, cols=cols0,  n_pool=1)  ;  # log(df2) 
           df2    = pd_add_easyid_bk(df2)  #### bk % 500 
                        
           dfk   = pd.concat((df2, df[ df.bk == bk] )) ; del df2 
           #dfk = df[df.bk== bk ]
           dfk   = dfk.groupby('easy_id' ).tail(10)
           neasy = dfk.easy_id.nunique()
           dirouti =  dirout + f"/user_eod_intra_{bk}_{tk}_{today1}_{len(dfk)}_{neasy}.parquet"
           pd_to_file( dfk,  dirouti, show=0)
        log(df)

        
    def daily_eod_user_hist_intra2(add_days=-1):  ### py  daily_eod_user_hist_intra
        ### T-2 End of day +  T-1  Intraday  --->  T-1 Full easyid click  for Rec Compute
        #### Most recent at bottom
        today1 = date_now_jp("%Y%m%d", add_days=add_days,  timezone= 'jp')
        tk     = get_timekey()+add_days
        ...
        for bk in range(0, 500):
           ...
           nk  = len(dfk)
           neasy = dfk.easy_id.nunique()
           dirouti =  dirout + f"/user_eod_intra_{bk}_{tk}_{today1}_{nk}_{neasy}.parquet"
    ...
    def daily_eod_user_sameday(bmin=0, bmax=100, add_days=0, split=0, full=1):     ### py daily_eod_user_sameday   >> "${logfile}_eod_sameday.py"  2>&1  &
        ...
        if split > 0 :
            cmd = f"{PYFILE} daily_eod_user_sameday --split 0  --add_days {add_days}  --full {full}   "
            batch_split_run(cmd= cmd, split=split, sleep= (2, 5) ) ; return 'done'
        ...
        log(df.shape)
        if full<1 : 
            dfeasy = set(df[ df['ts'] > time.time()-4*3600 ].easy_id.unique() ) ;  
            log('New easy', len(dfeasy))
        ...
        for bk in range(bmin, bmax):
           ...
           if full<1 : dfk = dfk[ dfk.easy_id.isin(dfeasy)] 
            
           dfk = dfk.groupby('easy_id' ).tail(10)
           nk    = len(dfk)
           neasy = dfk.easy_id.nunique() 
           dirouti =  dirout + f"/user_eod_intra_{bk}_{tk}_{today}_{nk}_{neasy}.parquet"
           pd_to_file( dfk,  dirouti, show=0)


    def daily_create_easyid_topg(mode='pur', add_days=0):   ###   py  daily_create_easyid_topg  --mode intra   2>&1 | tee -a zlog_brw.py   &
        ...
        log(df0 )
        diskcache_save2(df0, db_path  = db[dbname] , colkey='easy_id',  colvalue = 'top_genre',   ttl=ttl,  npool=8, verbose=False )
        log('finished')


    def daily_eod_user_pur_intra(add_days=0) :  ### py    daily_eod_user_pur_intra
        ### T-2 End of day +  T-1  Intraday  --->  T-1 Full easyid click  for Rec Compute
    ...
    def daily_eod_user_brw_intra(add_days=0) :  ### py  daily_eod_user_brw_intra    >> "${logfile}_faiss3.py"  2>&1   &&
        ### T-2 End of day +  T-1  Intraday  --->  T-1 Full easyid click  for Rec Compute
        ### siid  genre_id  n_pur       gms  price
        ###  siid  genre_id   n_pur         gms    price      gms_t1  n_pur_t1      gms_t2  n_pur_t2
        today0 = date_now_jp("%Y%m%d", add_days=  0 + add_days,  timezone= 'jp')
    ...
    ######
    def daily_create_index_faiss(add_days=0) :   ### py daily_create_index_faiss     >> "${logfile}_faiss4.py"  2>&1   &
    ...
    def daily_create_topk_batch(ii=0, ii2=500, tag='all', kbatch=20, uemb=1):
         ...
         if  ('new' in  tag  or 'all' in tag ) and len( os_process_find("python prepro_prod.py daily_eod_user --ii 0 ") ) > 0 :
         ...
         if 'rename' in tag and 'intra' not in tag:  ## Rename previous folder
            os_rename( dir_cpa3 + f"/hdfs/daily_usertopk/m001/{tk}/daily_user_eod/", tag="_")
                        
         elif 'intra' in tag : 
           os_rename( dir_cpa3 + f"/hdfs/daily_usertopk/m001/{tk}/intraday/out/", tag="_", ) 
         ...
         for jj in range(0, jmax ):
            cmd= f" {PYFILE}   daily_eod_topk2 --ii {jj*kbatch}  --ii2 { (jj+1)*kbatch } --tag {tag}  --uemb {uemb}  &  "  ###  2>&1 | tee -a '{logfile}'
            os.system(cmd)
            time.sleep(20)

                        
            
    def daily_eod_topk2(ii=0, ii2=100, tag='all', add_days= 0, uemb=1):   ### py daily_eod_topk2  --ii 0  --ii2 1 --tag new_rename  >> "${logfile}_00_eod_topk.py"  2>&1   &
        """  py daily_eod_topk2  --ii 0  --ii2 30 --tag new_rename  --add_days -1   --uemb 1 >> "${logfile}_00_eod_topk1.py"  2>&1   &
        ...
        # if 'no_addon' in tag :
        #    global TOPL_ADDON
        #    TOPK_ADDON = False  #### No addon in the rec list
        ...
        global ok_global ;   ok_global = ok_get_global_siid(add_days=add_days)
        global clientdrant ; drant_init( ndim=512 , efsearch= 32, add_days= add_days)


        log("############## Top-k ##################################################")
        bkmin = ii ; bkmax = ii2 ; kk =0 
        for bk in range(bkmin, bkmax):
            if 'test' in tag and kk > 3 : break
            flist = [] ; tk = tmax ; kk = kk + 1
            for dirin in dirin_list :
            ...
            for jj,fi in enumerate( flist) :
                ...
                log(fi)
                if 'intra' in tag : 
                   dirouti = dirout + f"/intraday/out/topk_{bk}_{tout}.parquet"
                   diroutj = dirout + f"/intraday/out/topk_{bk}_{tout}_*.parquet"
                ...
                if uemb > 0 :
                    ...
                    if len(dfe) < 1 : dfe = None                    
                    else :            log("#### Loaded user embed: ", dfe.shape, dirine)
                ...
                df2 = df2[ df2.topk_n > 10  ] ; log('  Cleaned topk', df2.shape)
                daily_eod_topk_logerror(df2, dirouti, dirout)
                log(df2[ [ci  for ci in df2.columns if "_n" in ci ] ])
                
                if  bk < 100 or  bk > 150  :
                    if bk not in  [0,1,10]  :
                       df2 = pd_del(df2, cols=['topk0', 'topk_genre', 'topk_emb'  ])
    ...
    def daily_eod_topk_logerror(df, finame, dirout):
        ### Check output
        ## #'easy_id', 'siid', 'siids', 'top_genre', 'topk', 'topk_emb', 'topk_emb_n', 'topk_genre', 'topk_genre_n', 'topk_n'],
        dd= {} ; err = False
        for ci in [ 'topk_n',  'topk0_n',  'topk_emb_n', 'topk_genre_n' ] :
        ...
        if err :
            msg = f"{finame} ,   {dd}"
            to_file(msg,  dirout +'/../topk_error.py')

            
    def daily_export(tag='eod', dirtarget='', add_days=0, bmin=0, bmax=500, split=0):  ### py daily_export  count  >> "${logfile}_00_export_count.py"  2>&1   &
        ### Create user list for Rec
        today = date_now_jp("%Y%m%d", add_days=0,  timezone= 'jp')
        tk    = get_timekey() + add_days
        
        flist = glob_glob(  dir_cpa3   + f"/hdfs/daily_usertopk/m001/{tk}/daily_user_eod/*" )
        if len(flist) < 500:
            log('Not 500 files', len(flist), flist)
            return
...
if 'daily_genre_score':
    ...
    def ca_campaign_bids(add_days=-1, mode=""):
...
if 'intraday, easyid topk':
    ...
    def intraday_eaysid_topk(test=0,) :     ### python prepro_prod.py intraday_eaysid_topk  --test 1   2>&1 | tee -a  "${logfile}_topk_intra_test.py"
        ...
        if now_hour_between(hour1="00:01", hour2="9:00", timezone="jp"):
        ...
        def isvalid(t,):
            tr = date_now_jp("%Y-%m-%d", timezone='jp', add_days= -1)   ### --20: 6am,  --21: 7am
            rref = [ tr + "--14",   tr + "--15", tr + "--16", tr + "--17", tr + "--18", tr + "--19", tr + "--20",   ]   ##  'in-progress'
            rref = rref + [ tr + "--21", tr + "--22",  tr + "--23",   tr + "--24", tr + "--00",  tr + "--01",    ]
            for r in rref:
                if r in t : return False
            return True
    ...
    def pd_easyid_get_topk(df, topk=50, cass_query=None, today1=None, faiss_index=None, faiss_map_idx=None, ivect=None, bk=0,dfe=None,  **kw):
        ...
        log('######## Embedding Rec ')
        df = create_rec_topemb3(df, faiss_index= faiss_index, faiss_map_idx= faiss_map_idx, ivect=ivect, maxrec=70, dfe=dfe)
        df = df.rename(columns={'topk': 'topk_emb'})


        log('\n###### Genre pur Rec: eaysid --> genre --> list of siid ')
        df = create_rec_topgenre(df, ngenre=5, topk=70)  ### topk_genre, topk_genre_n


        log("###### Merge, add on topk list   ")
        df = pd_add_ng_easysiid(df, bk=bk)  ####   'ng_siid' column, need the bucket bk to match !!!!!!!!
        global ok_global ;  ok_global = ok_get_global_siid(add_days=0)  if ok_global is None else ok_global
        
        
        #  nrec (25) << maxrec_adon (ie 40), so Top item with high CPC is added in the list.....
        #  15 items are added in the list
        df['topk0'] = df.apply(lambda x : pd_siid_combine6( x['topk_emb'], x['topk_genre'],
                                                           ngs= x['ng_siid']  if 'ng_siid' in x else set(),
                                                           ok_global = ok_global,
                                                           
                                                           ### nmax=18 Increase when end of month : reduce inventory
                                                           n1=9,  nmax= 30 )   , axis=1  )

        df = pd_topk_count( df, [ 'topk0',  'topk_emb', 'topk_genre',    ] )

        if TOPK_ADDON : df = create_rec_addon(df, topop= None, maxrec=40, colrec='topk0' )
        else : df = df.rename(columns={'topk0': 'topk'})

        df = pd_del(df, cols=['ng_siid', 'user_emb', 'topk_emb'  ])
        df = pd_topk_count( df, [ 'topk',    ] )
    ...
    ########################################################
    def create_rec_addon(df, topop=None, maxrec=40, ngs=None, colrec= 'topk0'):
        """ Add Missing topk
        """
        topop = rec_topopular()
        ...
        df['topk'] = df.apply(lambda x :  genre_add_on(x[colrec], x['ng_siid']  if 'ng_siid' in x else set() ), axis=1 )  #### Add Top popular for unknown users
        return df


    def rec_topopular():
        topop = db_ca_genre_siid['topk_popular']
        topop = topop[:70]  ### 100 : bad results
...
if 'combine, clean topk list':
    ...
    def pd_siid_combine5(t1, t2="", t3="", t4="", ngs=None, n1=0, n2=0, n3=0, n4=0, ok_global=None, nmax=51):
        ...
        use_global = True if len(ok_global) > 40000   else False 
        ...
        #### Item intra, item genre intra,  brw past, pur past
        ss = t1[:n1] + t2[:n2] + t3[:n3] + t4[:n4]
        ss = [t for t in ss if t not in ngs ]
        ii = len(ss)
        ss = ",".join(ss) + ","
        ...
        for x in chain.from_iterable(zip_longest(t1[n1:], t2[n2:], t3[n3:], t4[n4:], )):
            ...
            if not use_global :    
                if x is not None and x not in ngs and len(x)>8:                
                    ii = ii + 1
                    ss = ss + x + ","
            else :
                if x is not None and x not in ngs and len(x)>8  and x in ok_global :                
                    ii = ii + 1
                    ss = ss + x + ","
    ...
    def pd_siid_combine6(t1, t2="", t3="", t4="", ngs=None, n1=0, n2=0, n3=0, n4=0, ok_global=None, nmax=51):
        """ Remove similar genres, remove NG
        """
        t1 = t1.split(",")  ###  past pur
        t2 = t2.split(",")  ###  past brw
        ...
        #### Item intra, item genre intra,  brw past, pur past
        #ss = t1[:n1] + t2[:n2] + t3[:n3] + t4[:n4]
        ss = [t for t in t1[:n1] if t not in ngs ]
        ss = ss + [t for t in t2[:n2] if t not in ngs and t not in ss ]
        ss = ss + [t for t in t3[:n3] if t not in ngs and t not in ss ]
        ss = ss + [t for t in t4[:n4] if t not in ngs and t not in ss ]
        ...
        # if len(ss) < 1: return ""
        #vif ss[0] == "," : return ss[1:-1]
        return ",".join(ss)
    
    
    def pd_siid_combine5b(tg, ngs=None, n1=1, n2=0, n3=0, n4=0, n5=0, n6=0, ok_global=None, nmax=51):
        """ Remove similar genres, remove NG
        """
        ng = len(tg)
        if ng < 1: return ""
        ...
        t5 = tg[4] if ng >=5 else []
        t6 = tg[5] if ng >=6 else []
        
        #### NG easyid_siid
        if ok_global is None:    ok_global = set()
        elif len(ok_global) < 1: ok_global = set()
        use_global = True if len(ok_global) > 50000   else False    
        ...
        ss = t1[:n1] + t2[:n2] + t3[:n3] + t4[:n4] + t5[:n5]  + t6[:n6]
        ii = len(ss)
        
        for x in chain.from_iterable(zip_longest(t1[n1:], t2[n2:], t3[n3:], t4[n4:], t5[n5:], t6[n6:], )):
            # print("")
            if ii >= nmax: break
            if x is None   : continue
            if len(x) < 8  : continue
...
if 'daily_eval':
    ...
    def daily_update_wrong_vect(mode='itembad', add_days=0):  ####  python prepro_prod.py daily_update_wrong_vect  --mode itembad     2>&1 | tee -a zlog_wrong3.py &
        tk =get_timekey() 
        # tk= "*"        
        if 'itembad' in mode : dirin = dir_cpa3 + f"/hdfs/intraday/item_bademb/{tk}/"
        if 'outlier' in mode : dirin = dir_cpa3 + f"/hdfs/daily_usertopk/m001/{tk}/*_outlier/"
        ...
        df = pd_read_file(flist,  n_pool= 20, nfile=10000, nrows=500500500)
        df = df.drop_duplicates('siid')
        if 'genre_dist' in df.columns : df = df[df.genre_dist > 8.0 ]
        log(df.shape)
        
        df = pd_add_itememb(df, delete=True, show=1)        
        cass_update(df, table=model0['table'], prefix= model0['pref'], colkey="siid", colval="item_emb")
    ...
    def daily_get_allhisto(add_days=0, tag="scpur", nfile=20,  mode='verbose', iscouch=False):   ####
        ...
        if len(glob.glob( dirout + f"/ca_{mode}_{tag1}_*.parquet")) < 1  :
            ...
            log("#### Couch load  ################################################")
            df = pd_couch_load(df, tk1, iscouch=iscouch )   ### tk-1
    ...
    def daily_check_html(add_days=0, tag="scpur", nfile=20,  mode='eod', iscouch=True):   ####
        ...
        for j,x in dfa1.iterrows():
            ...
            topkemb1   = str(x['topk_emb']).split(",")[:50]    if 'topk_emb'    in x else  []
            topkgenre1 = str(x['topk_genre']).split(",")[:50]  if 'topk_genre'  in x else  []
            topk0      = str(x['topk0']).split(",")[:50]       if 'topk0'       in x else  []

            itopkemb1   = str(conv2(  topkemb1 ,    key)) 
            itopkgenre1 = str(conv2(  topkgenre1,   key)) 
            itopk0      = str(conv2(  topk0 ,      key))  
            ...
            doc.add('topk', topk )
            doc.add('topk0', itopk0 )
            doc.add('topkemb1', itopkemb1 )
            doc.add('topkgenre1', itopkgenre1 )
            ...
            doc.add('topk0',           topk0 )            
            doc.add( 'topkemb1',       topkemb1   )
            doc.add( 'topkgenre1',     topkgenre1 )
            ...
            doc.add( 'topkemb_genre',    topkemb1  , 'genre_path') 
            doc.add( 'topkgenre_genre', topkgenre1 , 'genre_path') 
...
if 'ab_test':
    def ab_cpc_calc(tag='eod', add_days=0): #### python prepro_prod.py   ab_cpc_calc  --tag eod    >> "${logfile}_abtest_eod_cpc_calc.py"  2>&1  &
        tk    = get_timekey() + add_days
        today = date_now_jp(add_days= add_days)
        ...
        cpc_dict  = ca_campaign_bids(add_days=add_days, mode='dict')

        flist = glob_glob(dirin, 5000)
        def cpc_calc(v, nmax=20):
            v1 = np.array([ cpc_dict.get(sid, 40.0)  for  sid in  v.split(",")[:nmax]  ] )
            return np.mean(v1)
        ...
        for fi in flist[:20] :
           ...
           # log(df['topk'])
           log(df.shape, fi )
           df['cpc']  = df['topk'].apply(lambda v: cpc_calc(v))
           df['cpc_full']  = df['topk'].apply(lambda v: cpc_calc(v, nmax=40))
        
           log( fi.split("/")[-1], df.cpc.mean(), df.cpc_full.mean(),  )
    ...
    def ab_extract_log(mode='imp,clk,pur', add_days=-1, tk = 18962):   
        #### python prepro_prod2.py   ab_extract_log --mode imp,pur,clk   --add_days -4     >> "${logfile}_abtest_click_export.py"  2>&1   &
        ### Export Daily CA Into specific folder,, avg rank of 6

        tka      = get_timekey() + add_days
        datea    = date_now_jp(add_days=add_days)

        import datetime
        datea1   = datetime.datetime.strptime(datea, '%Y%m%d')
        datecamp = datea1 - datetime.timedelta(days= datea1.weekday())
        datecamp = datecamp.strftime("%Y%m%d")


        ab_list =[ # [  18983 ,'20211220',  'kvn-20211222' ],   ### CTR 1.83, 0.5 CVR
                   [ tka, tka , datecamp,  f'kvn-{datea}' ],   ### CTR 1.83, 0.5 CVR
                 ]
        #ab_list =[ # [  18983 ,'20211220',  'kvn-20211222' ],   ### CTR 1.83, 0.5 CVR
        ...
        for aa in ab_list :
            ...
            logic1 = ca_logic_gethash(campaign_id= campid,  logic_id= logic_id )
